...
- [x] Vite/npm stack on host, then Node/Express and Python backend targets — **all three shipped** (Vite live-verified, Node with a real spawned server, Python with a real venv + server, 0.17.0). Design in [`docs/STACKS.md`](docs/STACKS.md): one *stack profile* (install/build/serve/entry/deployDir/doubleClick) read by every stage; 4 shippable steps; 3 open questions.
...
- [x] Homebrew / Docker packaging — `Dockerfile` (Playwright base, /data volume, verified), `homebrew/projectinator.rb` + `scripts/brew-formula.sh` (tap live: https://github.com/smanookian/homebrew-projectinator; `scripts/brew-formula.sh --push` publishes a release to it), `PROJECTINATOR_HOME`.
