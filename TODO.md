...
- [x] Docker packaging — `Dockerfile` (Playwright base, /data volume, verified), `PROJECTINATOR_HOME`. **Homebrew dropped in 0.22.1**: `brew install` fails on the vendored signed Mach-O addons and there's no opt-out — see `docs/INTERNAL.md`. npm is the install path.
