...
- [ ] Richer tester — **pieces 1–3 shipped** (`interact_app`: Tester-only click/fill/select/press/expect script, ≤20 steps, 10 s, stops at first failure with the reason, screenshot after) (a11y/basics: title, lang, h1, alt, labels, WCAG contrast on body+headings — `a11y.ts`, no model tokens) (3-viewport screenshots + overflow signal, saved to `.checks/`, listed in Transcripts, 1–3 opens them). Design agreed in [`docs/TESTER.md`](docs/TESTER.md): (1) 3-viewport screenshots + overflow signal, (2) six deterministic a11y/quality checks (not Lighthouse), (3) `interact_app` declarative step tool, (4) visual delta on rebuild. Three open questions there.
