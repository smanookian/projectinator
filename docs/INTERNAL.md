...
- **Estimates run ~2x low.** Calibration measures tokens correctly, but the cost math assumes the measured `cachedInputFraction` (~0.95) applies to billing; real runs behave like 0.5-0.75, so a $1 cap halted a build estimated at $0.82. Fix the cache assumption (or calibrate on cost) before trusting plan estimates.
