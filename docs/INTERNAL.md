...
- **Every screen is now audited for overflow** — the council epic-approval screen was the last (0.26.1) and was genuinely broken: its menu sits below an unbounded list, so ten epics pushed "Skip epics" off a 24-row frame. Any new screen that puts an actionable element after a list MUST budget the list by `termRows`.
...
- **No Homebrew, and don't try again**: `brew install` fails in Homebrew's post-install relocation pass. It runs `Keg#fix_dynamic_linkage` over every Mach-O file in the keg, and our tree vendors prebuilt *signed* ones (`@earendil-works/pi-coding-agent` → `@mariozechner/clipboard` → `clipboard.darwin-arm64.node`, plus Playwright). Rewriting the dylib ID invalidates the signature → `failed changing dylib ID` / `failed to fix install linkage` on Apple Silicon. There is no per-formula opt-out (`cellar: :any_skip_relocation` only affects poured bottles); the only workaround stages every `.node` outside the keg and re-`codesign`s it in `post_install`, which leaks files past `brew uninstall` and rots whenever a dep adds a native module. homebrew-core wouldn't take it either (vendoring rules; owner self-submission notability is 90 forks / 90 watchers / 225 stars). Install paths are **npm** (primary) and Docker.
- **Demo tour**: `npx tsx scripts/capture-tour.tsx` rewrites `docs/tour.svg` from real frames. Do NOT try to replace it with an asciinema/PTY recording without first fixing this: under a pseudo-terminal the TUI intermittently writes only the alt-screen switch and never a frame (same input gave 14 / 181 / 3104 bytes on consecutive runs); a real terminal is unaffected, and `ink-testing-library` is deterministic.
- **Build modes** (`guard.ts`): `checkToolCall()` is pure and holds every rule; `guardExtension()` wires it to Pi's `tool_call` hook (the only pre-execution veto point). Refusals set `terminate: false` so one blocked command does not end the task. Sessions load with `noExtensions: true` — the workspace is model-written and Pi would execute an extension found there. Safe is the default; `auto` disables the guard entirely. Not a sandbox: a build can still run a script it wrote inside its own folder.
- **Pi capabilities we deliberately do NOT use yet** (audited 0.27.0): `session.steer()` for mid-task redirection (today `BuildControl` only acts between launches), skills discovery from the workspace `.pi/skills` (may already work — verify before building), session `fork()` to reuse context on retry rounds instead of re-sending ~90k input tokens, and containerised execution (`containerization.md`) — model-written code currently runs on the host with `bash`. Auto-compaction we get for free; Pi compacts at a context threshold inside the run.
- **Tester vision**: screenshots go to the model via `PromptOptions.images`, gated on `model.input.includes("image")`, resized by Pi's `resizeImage`. Three paired live runs failed to show it catching anything the text-only Tester missed (it reads the CSS instead), so do not claim it catches more; its value is the cases source reading cannot reach.
- **Releasing**: bump + tag + `npm publish`. Users upgrade with `projectinator update`, which only self-installs when `installKind()` says `npm-global` (guards against npm-clobbering a clone or a project dep; Docker is told to rebuild).
- **User data**: `dataHome()` = `$PROJECTINATOR_HOME` or `~/.projectinator`; projects in `projects/`, Scout's routing overrides in `registry.overrides.json`. Docker sets `PROJECTINATOR_HOME=/data`. **All user data goes through `dataHome()`** (config, projects, `calibration.json`, `openrouter-models.json`, `templates.json`, `web/<provider>/` profiles) — `join(homedir(), ".projectinator")` appears only in `config.ts`, as the default and the legacy fallback. **Never write user data under `projectRoot()`** — `npm i -g` replaces that directory, so pre-0.22 installs lost every project on upgrade. `tuiRoot()` copies (never moves) anything left in the old `<root>/.workspace/tui` on first use. `test/setup.ts` pins `PROJECTINATOR_HOME` into the repo so tests can't touch real projects **or the real config** — `configPath()` follows `dataHome()` too, which it didn't before 0.22.1.
- **Never treat a stdout chunk as a line.** `serve.ts` buffers partial lines across chunks and flushes an unterminated tail. Splitting per chunk corrupted `listening on <port>` ~1 run in 6 (this was the "known flake" blamed on a port race in 0.23.3 — it wasn't) and would mangle any stack trace read from the log. `test/serve-log.test.ts` reproduces it deterministically with a two-write banner.
- **`loadConfig()` must not filter fields.** It used to return a hand-listed projection, so five prefs (task limits, parallelCode, reviewPolicy, webhookUrl) were saved and dropped on read. Add a pref to `AppConfig`/`getPrefs`/`setPrefs` only — never a read-side allowlist.
- **A fresh build must call `uniqueDir()`.** `startBuild` without `opts.workspace` allocates the next free folder; reusing the slug overwrote an existing project's state and files. Resume/change pass `workspace` deliberately.
- **A judge with no verdict is a failure, not a pass.** `runOne` turns a verdict-less `test`/`review` outcome into an error + halt; the feedback loop keys on `verdict && !passed`, so without that a silent judge certified the build.
- **Cost estimates come from measured USD, not token math.** `recordActual` stores `usd` per bucket/model; `route()` uses `calibratedCostUSD` after 2 runs, else prices tokens with `cachedInputFraction: 0`. Do NOT restore the cache-discounted fallback: usage stats report ~95% cacheRead but bills behave like 0%, which made estimates ~2x low (est $1.35 → actual $2.46) and halted builds against their own cap. The fallback is intentionally high rather than low. Calibration measures tokens correctly, but the cost math assumes the measured `cachedInputFraction` (~0.95) applies to billing; real runs behave like 0.5-0.75, so a $1 cap halted a build estimated at $0.82. Fix the cache assumption (or calibrate on cost) before trusting plan estimates.
- **Review policy** (`ReviewPolicy` in `types.ts`, pref `reviewPolicy`, default `high`): enforced in `normalizeBacklog()` because the PM prompt alone is not binding. Keys on the *reviewed code task's* difficulty — review tasks are low by construction. Dropping a review makes its dependents inherit its code deps, so the test never runs before the code. Evidence for the default: reviews were 22% of spend / 0 bugs over 4 runs; Tester averaged $0.02 and actually executes the app.
- **The budget cap is a soft cap.** `runBacklog` clears each task against its *routed estimate* before launching (`running + reserved + est > cap` → halt), so a build whose actuals beat the estimates lands above the cap — a real $1-cap run billed $1.20. Enforcing it hard would mean killing a task mid-flight and paying for it anyway. The CLI explains the overshoot instead.
