...
- **Known flake (seen once, unreproduced)**: `test/python.test.ts` "creates the venv once … serves main.py" failed in one full-suite run and passed 8 runs in a row afterwards (3 full, 5 targeted). `freePort()` in `serve.ts` probes a free port and the child binds it *later*, so a parallel suite can lose the race. Not fixed speculatively — if it recurs, capture the thrown message first: "exited with code N before listening" means the bind race, "did not answer within Ns" means a slow start.
...
- **Estimates run ~2x low.** Measured again on a completed build: est $1.35, actual $2.46 (1.8x). Calibration measures tokens correctly, but the cost math assumes the measured `cachedInputFraction` (~0.95) applies to billing; real runs behave like 0.5-0.75, so a $1 cap halted a build estimated at $0.82. Fix the cache assumption (or calibrate on cost) before trusting plan estimates.
